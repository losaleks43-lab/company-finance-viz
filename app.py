...
# VERSION: Universal / Industry-Agnostic
...
def extract_text_from_uploaded_file(uploaded_file) -> str:
    ...
    - Favors 'Consolidated' tables (Universal).
    - Favors 'Segment' tables (Universal).
    - Penalizes 'Standalone' tables (prevents the Reliance/Conglomerate error).
    """
        ...
        try:
            ...
            for i in range(num_pages):
                try:
                    ...
                    # 3. Penalize Standalone (Crucial for non-US conglomerates)
                    for kw in negative_keywords:
                        if kw in low_text: score -= 15
                        
                    # 4. Context Boost (Look for the actual data columns)
...
# -------------------------------------------------------------------
# 2. Adaptive AI Extraction (The "Analyst")
# -------------------------------------------------------------------
...
def extract_pnl_with_llm(raw_text: str):
    """
    Universal Prompt: Asks the AI to identify the business model first, 
    then extract the appropriate lines for that model.
    """
    ...
    4. **TAX**
       - Extract "Tax Expense" or "Current + Deferred Tax".
